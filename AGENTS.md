...
Valkey/Redis cache services: locking, rate limiting, pub/sub, streams, delayed jobs.

- Repository path: `packages/java/cache` (relative to the Omnixys root)
...
src/main/java/com/omnixys/cache/{core, service, model, health}
...
Testcontainers-based tests in src/test; integration tests require Docker.
